...
// ---------------- UTIL ------------------
function loadProductsFromJSON(): IProduct[] {
...
app.get("/api/products", async (req, res) => {
  try {
    ...
    if (cached && !all) return res.json(JSON.parse(cached));
    ...
    if (!all) await redis.set(redisKey, JSON.stringify(response), "EX", 600);
    res.json(response);
  } catch (err) {
    console.error(err);
...
async function scrapeShopify() {
  ...
  const urls = Array.from({ length: totalPages }, (_, i) =>
    `${process.env.SHOPIFY_COLLECTION_URL}?page=${i + 1}`
  );
  ...
  const crawler = new PlaywrightCrawler({
    ...
    async requestHandler({ page, pushData, request }) {
      try {
        ...
        console.error(`Error scraping ${request.url}:`, (err as Error).message);
      }
    }
  });
  ...
  await redis.set(
    "shopify_products",
    JSON.stringify({ success: true, total: allProducts.length, products: allProducts }),
    "EX",
